...
By calculate the first- and second-order derivatives of $U$ in terms of $s_t$, we can obtain$$\tag{F1}
\frac{\partial U}{\partial s_t} = w_tv'(s_t)(v(s_t)+1-U)
$$and$$\tag{F2}
\frac{\partial^2 U}{\partial s_t^2} = w_t[(v(s_t)+1-U)((1-2w_t)v'(s_t)^2+v''(s_t))+v'(s_t)^2]
$$

We can show the solution to Equation (5) is a decreasing reward sequence, i.e. $s_0\geq s_1\geq ...\geq s_T$ and there exists at least one period $t$ such that $s_t>s_{t+1}$. To see this, consider for any $0\leq t+1\leq T$, we have $s_{t+1}=s_t$. By Equation (F1), in that case we can obtain $\frac{\partial U}{\partial s_{t+1}}/\frac{\partial U}{\partial s_{t}}=\delta<1$. This indicates transferring an incremental consumption from period $t+1$ to $t$ will increase the total value of consumption $U$, while doing from period $t$ to $t+1$ will decrease $U$. Hence, to maximize $U$, the DM's consumption plan must be a decreasing reward sequence.

Now we look at the first part of Proposition 6.
