...
\alpha e^{-u}- e^{-\alpha u} -(1-\delta)(\alpha-1)$$ from eq. (1) we
know the the left hand equals
$\delta^{-t_1}\alpha e^{-u}-\delta^{-t_2}e^{-\alpha u}$. Therefore, the
inequality always holds when $\Delta t>0$
...
(Lama 1) for any $x_1<x_2<\infty$, there always exists $t_1<t_2<\infty$
...
5.  magnitude-dependent hyperbolic (HB-MD)
...
7.  quasi-hyperbolic plus fixed delay cost (QuasiHB-FC)
