...
**Definition 2**: $\succsim$ has an attentional discounted utility
...
where $\{w_t(X_T)\}$ and $\{w_t(Y_{T'})\}$ are generated by constrained
...
Axiom 2: (spread-stationarity correlation) $c\sim X_T$ implies
$X_T \sim cX_T$
...
$c'$, $X_T'$, $cX_T\succsim c'X_T'$, where
$c+\sum_{t=0}^Tx_t=c'+\sum_{t=0}^Tx_t'$, then $X_T \succsim X_T'$ when
and only when $c\sim X_T$, where $\sum_{t=0}^Tx_t=\sum_{t=0}^Tx_t'$.
...
## Hidden Zeros and Common Difference Effect
