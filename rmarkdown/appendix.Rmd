...
**Lemma A.1**: Suppose $\succsim$ has an ADU representation, $w_t$ is the weight for period delivering reward $s_t$ in $cS_T$, $w'_t$ is the weight for period delivering reward $s_t$ in $S_T$. Then Axiom 2 implies, for any $c$ and $S_T$, we have $w'_t=w_t/\sum_{t=0}^Tw_t$.
