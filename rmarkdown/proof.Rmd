...
For sequence $s_{0\rightarrow T}$, we can iteratively apply these
...
$w_t >0$, $w'_t>0$, $\sum_{t=0}^Tw_t=1$ and $\sum_{t=0}^{T+1}w'_t=1$, we
must have $\frac{w_0}{w'_0}=\frac{w_1}{w'_1}=…=\frac{w_T}{w'_T}$.
...
Given instantenous utility can be any non-negative real number, we must
have $w_t=w'_t$.
