...
Characterization}\label{axiomatic-characterization}}
...
\hypertarget{explaining-non-stationary-time-preferences}{%
\section{\texorpdfstring{Explaining Non-Stationary Time Preferences
\label{behavioral}}{Explaining Non-Stationary Time Preferences }}\label{explaining-non-stationary-time-preferences}}

\hypertarget{evaluating-a-delayed-reward}{%
\subsection{Evaluating A Delayed
Reward}\label{evaluating-a-delayed-reward}}

Suppose there is a reward sequence with a time length of \(T\). A
decision maker receives a positive detereminstic reward in time period
\(j\) (and no reward in other periods), that is, for any
...
& \delta^j \frac{1}{1+\frac{\delta}{1-\delta}(1-\delta^T)e^{-v(x_j)}}\;, & 0<\delta<1 \\
& \frac{1}{1+T\cdot e^{-v(x_j)}}\; , & \delta=1
\end{aligned}
\right.
\]

When \(\delta=1\), the attention weight \(w_j\) takes a form similar to
hyperbolic discounting.

Therefore, the value of this delayed reward is \(w_Tu(x_T)\). By
Equation (1),\[ \tag{2}
w_T  =\frac{1}{1+G(T)\cdot e^{-u(x_T)/\lambda}}
\] where\[
...
that Equation (2) can explain a series of experimental findings.
...
\hypertarget{explaining-dynamic-inconsistent-behavior}{%
\section{Explaining Dynamic Inconsistent
Behavior}\label{explaining-dynamic-inconsistent-behavior}}
...
\hypertarget{consumption-over-smoothness-and-over-sensitivity}{%
\section{Consumption Over-Smoothness and
Over-Sensitivity}\label{consumption-over-smoothness-and-over-sensitivity}}
