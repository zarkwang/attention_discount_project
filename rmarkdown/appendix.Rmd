...
u_t-\frac{\partial C}{\partial w_t}=\theta
$$Given that $C$ is strictly convex in $w_t$, and $u_t$ is increasing in $s_t$, we know that $w_t$ is also increasing in $s_t$. For this reason, we can denote the solution of this constrained optimal discounting problem by $w_t\equiv\eta_t(s_t;\theta)$, where $\partial \eta_t/\partial s_t>0$.
...
To prove Lemma A.1, from $c\sim S_T$, we know$$\tag{A.3}
u(c)=\sum_{t=0}^T w_t u_t
$$From $S_T\sim cS_T$ and Equation (A.3), we know$$\tag{A.4}
u(c)=w_c'u(c)+\sum_{t=0}^{T} w_t'u_t
$$where $w'_c$ is the weight for the present period (which delivers reward $c$) in sequence $cS_T$. Given that $w'_c=1-\sum_{t=0}^T w'_t$, we can rearrange Equation (A.4) to$$\tag{A.5}
