...
$e_v(x)$ is the elasticity of $v(x)$ to $x$.
...
**Proposition 4**: If $\delta =1$, then the discount function is convex
in $t$. If $0<\delta<1$, then there are a reward threshold
...
$V(x,t)$ is strictly concave in $x$ when $x\in [\bar{x},+\infty)$

(2) if $\frac{d}{dx}\left(\frac{1}{v'(x)}\right)$ is continuous at
$x=0$, and $\frac{d}{dx}\left(\frac{1}{v'(0)}\right)<1$, then there
exists a threshold $x^*$ in $(0, \bar{x})$ such that, for any
$x\in (0,x^*)$, $V(x,t)$ is strictly convex in $x$ .

(3) there exist a hyper-parameter $\lambda^*$ and an interval
$(x_1,x_2)$ such that, if $\lambda<\lambda^*$, for any $x\in(x_1,x_2)$,
$V(x,t)$ is strictly convex in $x$, where $\lambda^*>0$ and
$(x_1,x_2)\subset(0,\bar{x})$.
