...
\textbf{Proposition 5}: Suppose \(t\geq1\),
...
\def\labelenumi{(\arabic{enumi})}
\item
  there exists a threshold \(\bar{x}\) in \((0,+\infty)\) such that
  \(V(x,t)\) is strictly concave in \(x\) when
  \(x\in [\bar{x},+\infty)\)
\item
  if \(\frac{d}{dx}\left(\frac{1}{v'(x)}\right)\) is continuous at
  \(x=0\), and \(\frac{d}{dx}\left(\frac{1}{v'(0)}\right)<1\), then
  there exists a threshold \(x^*\) in \((0, \bar{x})\) such that, for
  any \(x\in (0,x^*)\), \(V(x,t)\) is strictly convex in \(x\) .
\item
  there exist a hyper-parameter \(\lambda^*\) and an interval
